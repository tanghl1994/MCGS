...
plt.subplot(1,2,1)
file_name_tree = 'final_linearly_speedup_result_graph.txt'
w_tree = 'final_linearly_speedup_result_graph_w.txt'
...
for a in alist:
    b = []
    f_tree = open('final_linearly_speedup_result_graph.txt')
    ...
    for k in range(4):
        ...
        for i in range(tree_max-1):
            ...
            if start>=a and end<=a:
                ...
                ma = max([sum([1 for i in range(int(l)) if w[i] == k]) for k in range(m)])
...
plt.savefig('linearly_speedup_graph.pdf')
