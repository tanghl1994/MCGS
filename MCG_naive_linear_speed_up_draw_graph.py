...
b = []
f_tree = open('final_linearly_speedup_result_MCG_graph.txt')
...
b = []
f_tree = open('final_linearly_speedup_result_naive_graph.txt')
lines = f_tree.readlines()
line = lines[0].strip().split(',')
for i in range(tree_max-1):
    start = float(line[i])
...
plt.savefig('MCG_naive_linearly_speedup_graph.pdf')
